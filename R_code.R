...
# Influential Points
influencePlot(model, id.n = 3, main = "Influence Plot", sub = NULL,
              identify = 2, scale = 0.5, main.title = NULL)
...
ols_plot_resid_stud_fit(model)
ols_regress(Birthweight ~ Age + SocialClass + BMI + Folate + VitC + VitB6 + VitB12 + VitE,
...
# Recode the variable Birthweight
birthweight_data$Underweight <- ifelse(birthweight_data$Birthweight <= 2800,
                                       "Underweight", "Normal")

# Recode as binary response
birthweight_data$Response <- ifelse(birthweight_data$Birthweight <= 2800, 1, 0)
...
# Comparison btw Poisson and nb (Final winner)
vuong(model_ZINB, model_zip)
